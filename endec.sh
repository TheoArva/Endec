...
function encrypt { #Encrypt zipped file & delete zipped file.

gpg --symmetric --cipher-algo AES256 "$response".tar.gz && gpgconf --reload gpg-agent;

rm -r "$response" "$response".tar.gz
...
if [[ $? -eq 0 ]]
then
	printf "\n\t!!!File or Folder does NOT exist!!!\n\nFor every incorrect entry, an empty \"*.tar.gz\" file is created.\nDelete it permanently (y/n) ?\n\n\a";
	sudo find ./ -type f -size -46b -amin -1 -name "*.tar.gz" -exec rm -i {} \; ; rm -r ~/.endecSTDERR.txt
else
	return 0
fi
...
then
	printf "\n\t!!!File or Folder does NOT exist!!!\n\nFor every incorrect entry, an empty \"*.tar.gz\" file is created.\nDelete it permanently (y/n) ?\n\n\a";
	sudo find ./ -type f -size -46b -amin -1 -name "*.tar.gz" -exec rm -i {} \; ;  rm -r ~/.endecSTDERR.txt 2> /dev/null
else
	encrypt 1> /dev/null 2> /dev/null
fi

}

function zipcrypt { #Final script to Zip & Encrypt file/folder.
